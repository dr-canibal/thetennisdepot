...
class TBT_Testsweet_Model_Test_Suite_Magento_Module_Conflict extends TBT_Testsweet_Model_Test_Suite_Abstract {
    ...
    protected function generateSummary() {
        ...
        if (empty($conflicts)) {
            ...
            $this->addWarning("Conflict(s) found");

            foreach ($conflicts as $conflict) {
                if (strpos(print_r($conflict, true), 'TBT_') > 0) {
                    $this->addFail("Rewrite conflict found that causes issues in the TBT namespace");
                    break;
                }
            }
        }
        ...
        foreach ($conflicts as $conflict) {
            ...
            if (count($conflict) == 2) {
                ...
                $string .= $this->__("5. backup and open: ") . "./app/etc/modules/{$child['class']['module_namespace']}_{$child['class']['module_name']}.xml\n";
