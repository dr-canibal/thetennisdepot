...
class Eznewsletter_Ezsubscribers_Model_Ezutilities_Api extends Mage_Api_Model_Resource_Abstract
{
    ...
    public function getMinOrderDate()
    {
		...
		$storeTableName = $resource->getTableName('sales/order');
		$readresult = $read->query("SELECT min(created_at) minOrderDate FROM $storeTableName");

		$minOrderDate = "";
		while ($row = $readresult->fetch())
		{
			$minOrderDate = $row['minOrderDate'];
		}
		$dateArr = explode(" ", $minOrderDate);
