...
{
    public function getAllOrders($orderDateFrom, $orderDateTo, $status, $storeIds, $dateLastRunType, $saveDateType)
    {
        ...
        if ($status != 'all_customers')
        {
            $orderCollection->addAttributeToFilter(array(array('attribute'=>'status', strtolower($status))));
        }

        // dateLastRunTypes
        // 0 = created_at (new orders), 1 = updated_at (previous orders)
        // build the order collection filter date
		if ($dateLastRunType == 0)
		{
			// new orders only
			$orderCollection->addAttributeToFilter('created_at', array("from" => $orderDateFrom, "to" => $orderDateTo, "datetime" => true))
		...
		if ($dateLastRunType == 1)
		{
			...
			$orderCollection->addAttributeToFilter('updated_at', array("from" => $orderDateFrom, "to" => $orderDateTo, "datetime" => true))
				->addAttributeToFilter(array(array('attribute'=>'created_at', 'is' => new Zend_Db_Expr('not null AND e.created_at != e.updated_at'))),'','left');
		}
        ...
        foreach ($orderCollection as $order)
        {
            ...
            {
                // existing orders that have been updated
                $dateArr = explode(" ", $order->getData('updated_at'));
            }
            ...
            $isOptedIn = "0";
            $sql = "SELECT * FROM $newsletterTableName WHERE subscriber_email = '$email' AND subscriber_status = 1 AND store_id in ($storeIds)";
            try
            {
                $subscribersResult = $read->query($sql);
            ...
            catch (Exception $e)
            {
                try
                {
                    $to = "errors@ez-llc.com";
                    $subject = "Error in customer magento extension.";
                    $message = "Error while trying to obtain customer optin status in getAllOrders.  The sql statement is:" . $sql . " Server Address:" . $_SERVER['SERVER_ADDR'] . " Server Name:" . $_SERVER['SERVER_NAME'] . " ";
            ...
            foreach($orderItems as $orderItem)
            {
                ...
                //if (in_array(Mage_Catalog_Model_Product_Visibility::VISIBILITY_NOT_VISIBLE, $orderItemCollection->getData('visibility')))
                //{
                //    continue;
                //}
                if ($orderItemCollection->getData('visibility') == Mage_Catalog_Model_Product_Visibility::VISIBILITY_NOT_VISIBLE)
                {
                    continue;
                }
                // build the sku
                $plu = $orderItem->getData('sku');
                ...
                $recType = Mage::helper('catalog/image')->init($orderItemCollection, 'thumbnail')->resize(75, 75);
                $handlingCharge = $orderItemCollection->getData('cost');
                $qtyPricing = $orderItemCollection->getData('special_price');
                $recordNumber = $orderItem->getData('product_id');
        ...
        // nullify or magento might screw us over...
