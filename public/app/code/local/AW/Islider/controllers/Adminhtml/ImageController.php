...
class AW_Islider_Adminhtml_ImageController extends Mage_Adminhtml_Controller_Action {
    ...
    protected function saveimageAction() {
        ...
        // Additional checks
        $_datesCheck = true;
        if($_data['active_from'] && strtotime($_data['active_from']) < 1) {
            $_errors[] = $this->__('Wrong value for \'Date From\' field');
            $_datesCheck = false;
        }
        if($_data['active_to'] && strtotime($_data['active_to']) < 1) {
            $_errors[] = $this->__('Wrong value for \'Date To\' field');
            $_datesCheck = false;
        }
        if($_data['active_from'] && $_data['active_to'] && $_datesCheck && strtotime($_data['active_to']) < strtotime($_data['active_from']))
