...
class AW_Ajaxcartpro_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function renderBigCart() {
	 ...
	 /* TBT_Rewards compatibility */
	if(!Mage::getStoreConfigFlag('advanced/modules_disable_output/TBT_Rewards')){
		 $points_slider = $L
                	->createBlock('rewards/checkout_cart_slider')
                	->setTemplate('rewards/checkout/cart/slider.phtml')
       		 ;


        	$minicart_js = $L
                	->createBlock('core/template')
                	->setTemplate('rewards/checkout/minicart_js.phtml')
        	;


      		$reward = $L
                	->createBlock('rewards/checkout_cart')
                	->setTemplate('rewards/checkout/cartmini.phtml')
			->setChild('points_slider', $points_slider)
                	->setChild('minicart_js', $minicart_js)
        	;

 		$catalogpoints = $L
                	->createBlock('rewards/checkout_cart_items')
                	->setTemplate('rewards/checkout/cart/items.phtml')
        	;

 		$catalogpoints_header = $L
                	->createBlock('rewards/checkout_cart_headers')
                	->setTemplate('rewards/checkout/cart/headers.phtml')
        	;

		$cart1 
			->setChild('cartpoints_spender', $reward)
                	->setChild('catalogpoints',  $catalogpoints)
                	->setChild('catalogpoints-header', $catalogpoints_header)
		;
	}
