...
class KD_ExpressCheckout_Model_Type_Standard
{
    /**
    ...
    private function _getDefaultAddress()
    {
        ...
        if (($customer = Mage::getSingleton('customer/session')->getCustomer())
            ...
            if (Mage::getStoreConfig('expresscheckout/geo_ip/country')) {
                $gi = geoip_open(
                    Mage::getBaseDir('lib')
                        . DS
                        . "MaxMind/GeoIP/data/"
                        . Mage::getStoreConfig('expresscheckout/geo_ip/country_file'),
                    GEOIP_STANDARD
                );
            ...
            if (Mage::getStoreConfig('expresscheckout/geo_ip/city')) {
                $gi = geoip_open(
                    Mage::getBaseDir('lib')
                        . DS
                        . "MaxMind/GeoIP/data/"
                        . Mage::getStoreConfig('expresscheckout/geo_ip/city_file'),
                    GEOIP_STANDARD
                );
            ...
            if (empty($result['shipping']['country_id'])) {
                $result['shipping']['country_id'] =
                    $result['billing']['country_id'] = Mage::getStoreConfig('expresscheckout/general/country');
            }
        }
    ...
    public function applyPaymentMethod($methodCode = null)
    {
        ...
        if (!$count = count($availablePayments)) {
            ...
            if (!$found || !$methodCode) {
                $methodCode = Mage::getStoreConfig('expresscheckout/general/payment_method');
                foreach ($availablePayments as $payment) {
                    if ($methodCode !== $payment->getCode()) {
                        continue;
                    }
    ...
    public function applyShippingMethod($methodCode = null)
    {
        ...
        if (!$count = count($rates['items'])) {
            ...
            if (!$found || !$methodCode) {
                $methodCode = Mage::getStoreConfig('expresscheckout/general/shipping_method');
                foreach ($rates['items'] as $rate) {
                    if ($methodCode === $rate['code']) {
                        $this->getQuote()->getShippingAddress()->setShippingMethod($methodCode);
                        $found = true;
    ...
    public function getCheckoutMethod()
    {
        ...
        if (!$this->getQuote()->getCheckoutMethod()) {
            if (Mage::helper('expresscheckout')->isAllowedGuestCheckout()) {
    ...
    public function validate()
    {
        ...
        if ($quote->getCheckoutMethod() == self::METHOD_GUEST && !Mage::helper('expresscheckout')->isAllowedGuestCheckout()) {
    ...
    {
	$version_array = Mage::getVersion();
	if($version_array['major']=='1' && $version_array['minor']=='4' && $version_array['revision']=='1')
    {
        ...
        /**
         * @var KD_ExpressCheckout_Model_Service_Quote
         */
        $service = Mage::getModel('expresscheckout/service_quote', $this->getQuote());
        $service->submitAll();
    ...
    public function validateAddress($address)
    {
        ...
        $address->implodeStreetAddress();
        $formConfig = Mage::getStoreConfig('expresscheckout/address_form');
