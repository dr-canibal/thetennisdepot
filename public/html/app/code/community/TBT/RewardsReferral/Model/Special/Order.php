        ...
        $hidescript = "
            function checkReferralFields() {
                ...
                if(v == 'customer_referral_order') {
                    //$('rule_points_amount').up().up().show(); 
                    $('rule_points_amount').up().up().cells[0].down().innerHTML = '{$this->getReferralOrderCaption()}';
                    //$('rule_points_amount_referral_first_order').up().up().show();
                } else {
                    $('rule_points_amount').up().up().cells[0].down().innerHTML = '{$this->getDefaultCaption()}';
                    //$('rule_points_amount_referral_first_order').up().up().hide();
                }
            }
            $('rule_points_conditions').onchange = function() {
                checkReferralFields();
            }
            checkReferralFields();
        ";
