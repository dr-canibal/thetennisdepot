...
//     		}
     ...
     */
    public function calculateCartPoints($rule_id, $order_items, $allow_redemptions)
    {
        ...
        if ($rule->getId()) {
            if ($crActions->isGivePointsAction($rule->getPointsAction())) {
                ...
                //@nelkaake Added on Friday June 11, 2010: pass salesrule to function in last param so that it knows to check validation
                $price = Mage::helper('rewards/transfer')->getTotalAssociatedItemPrice($order_items, $rule->getId(), $rule);
                ...
                if(!empty($order_items)) {
                    if($order_items instanceof TBT_Rewards_Model_Sales_Quote 
                        || $order_items instanceof TBT_Rewards_Model_Sales_Order) {
                        foreach($order_items as &$item) {
                            $first_item = $item; break;
                        }
                    } elseif (is_array($order_items)) {
                        $first_item = $order_items[0];
                    }
                    if($first_item->getQuote()) {
                        $additional = $first_item->getQuote()->getTotalBaseAdditional(); 
                    } else {
                        $additional = $first_item->getOrder()->getTotalBaseAdditional(); 
                    }
                    $price += $additional;
                }
    ...
    {
    	if($cart == null)  	$cart = $this->getQuote();
    	$points = array();
        $points_exist = false;
   ...
   /**
    ...
    public function collectShoppingCartRedemptions($cart=null)
    {
        ...
        foreach (explode(',', $cart->getAppliedRedemptions()) as $rule_id) {
            $rule = Mage::helper('rewards/transfer')->getSalesRule($rule_id);
            if (!$rule_id || !$rule->getId()) {
                continue;
            }
            $is_redemption_rule = Mage::getModel('rewards/salesrule_actions')
        ...
        $ret = array(
            'applied'=>$applied_redemptions, 
            'applicable'=>$applicable_redemptions,
            'sorted_applicable'=>$sorted_applicable,
   ...
   /**
    ...
    private function getCartRedemptionsMap($cart=null) {
        ...
        foreach($cart_redemptions as $redem_id) {
            if(empty($redem_id) && $redem_id != 0) continue;
...
//    	Mage::helper('rewards/debug')->dd($total_cart_points);
    ...
    public function getTotalPointsSpendingAsStringList() {
       	...
       	$points_string = Mage::getModel('rewards/points')->set($points_spending)
       						->getRendering()->setDisplayAsList(true);
       	return $points_string;
    }
