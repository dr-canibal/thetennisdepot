...
class TBT_Rewards_Model_Observer_Catalog_Product_Flat_Update_Product
{
	...
	public function updateCatalogRulesHash($observer)
	{
		...
		if($action) {
            ...
            if(!$product_id) $action = null; //if no product id available, reset our assumption because this must be some other unrecognized request.
        }
        
		// TODO WDCA: this does not work with other-language stores - use plain old now(true) instead
	    $now = Mage::helper('rewards')->now();
        ...
        $select = $read->select()
            ->from( $catalogrule_price_table, array('product_id','customer_group_id', 'website_id'))
            ->where('rule_date = ?', $now);
            
        //@nelkaake Friday March 12, 2010 03:48:20 PM : if this was a product save/delete/update request, only update that product.
        if($action) $select->where('product_id = ?', $product_id); 
        ...
        foreach ($collection as $row) {
			...
			$associated_rule_ids = $this->_getAssociatedRules($product_id, $website_id, $customer_group_id);
        ...
        //Mage::log("rewards applied rules on product..");
		return $this;
	}
	 ...
	 * @return array()
	 */
	protected function _getAssociatedRules($product_id, $wId=null, $gId=null) {
		if ( isset($this->associated_rule_ids_by_product[$product_id]) ) {
		    $associated_rule_ids = $this->associated_rule_ids_by_product[$product_id];
		}
		else {
		    $associated_rule_ids = Mage::getModel('rewards/catalog_product')
		    		->setId($product_id)
                	->getCatalogRuleIds($wId, $gId);
            ...
            $this->associated_rule_ids_by_product[$product_id] = $associated_rule_ids;
		}
