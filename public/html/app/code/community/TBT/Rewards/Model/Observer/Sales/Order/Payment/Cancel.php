        ...
        if (Mage::helper('rewards/config')->shouldRemovePointsOnCancelledOrder()) {
            ...
            foreach($orderTransfers as $transfer) {
                if ( ($transfer->getStatus() == TBT_Rewards_Model_Transfer_Status::STATUS_PENDING)
                        || ($transfer->getStatus() == TBT_Rewards_Model_Transfer_Status::STATUS_ON_HOLD) ) {
                	...
                	try {
		                $is_transfer_successful = Mage::helper('rewards/transfer')->transferRevokedPoints(
		                    $transfer->getQuantity() * -1,
		                    $transfer->getCurrencyId(),
		                    $transfer->getId(),
