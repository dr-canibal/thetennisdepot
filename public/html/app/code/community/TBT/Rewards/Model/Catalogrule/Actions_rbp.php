...
{	
	...
	const DEDUCT_BY_PROFIT_SPENT_ACTION      		= 'deduct_by_profit_spent';

    public function getOptionArray()
    {
        return array_merge(
        	$this->getRedemptionOptionArray(), 
        	array('' => Mage::helper('rewards')->__('--  --')), 
        	$this->getDistributionOptionArray()
        );
    }
    ...
    {
        return array(
            self::DEDUCT_POINTS_ACTION => Mage::helper('rewards')->__('Spends X Points'),
            self::DEDUCT_BY_AMOUNT_SPENT_ACTION => Mage::helper('rewards')->__('Spends X points for every Y dollar amount in price'),
            self::DEDUCT_BY_PROFIT_SPENT_ACTION => Mage::helper('rewards')->__('Spends X points for every Y dollar amount in profit'),
        );
    }
    public function getDistributionOptionArray()
    {
        return array(
            self::GIVE_POINTS_ACTION => Mage::helper('rewards')->__('Give X points to customer'),
            self::GIVE_BY_AMOUNT_SPENT_ACTION => Mage::helper('rewards')->__('For every Y dollar amount in price, give X points'),
        );
    }
    /**
     ...
     * @return boolean
     */
    public function isDeductByProfitSpentAction($action) {
    	return $action == self::DEDUCT_BY_PROFIT_SPENT_ACTION;
    }
    ...
    public function getRedemptionActions() {
    	return array(
    		self::DEDUCT_POINTS_ACTION, 
    		self::DEDUCT_BY_AMOUNT_SPENT_ACTION, 
    		self::DISCOUNT_BY_POINTS_SPENT_ACTION,
    		self::DEDUCT_BY_PROFIT_SPENT_ACTION, 
    	);
    }
    ...
    public function getDistributionActions() {
    	return array(self::GIVE_POINTS_ACTION, self::GIVE_BY_AMOUNT_SPENT_ACTION);
    }
