...
class TBT_Rewards_Model_Newsletter extends Mage_Newsletter_Model_Subscriber
{
    ...
    public function save()
    {
        ...
        if($this->isSubscribed() && !$was_subscribed){
			Mage::dispatchEvent('rewards_newsletter_new_subscription', array('subscriber'=>&$this));
    ...
    public function getPredictPoints(){
    	Varien_Profiler::start("TBT_Rewards:: Predict Newsletter Points");
    	$ruleCollection = Mage::getSingleton('rewards/special_validator')->getApplicableRulesOnNewsletter();
    	$predict_array = array();
        foreach($ruleCollection as $rule){
        	$predict_array[$rule->getPointsCurrencyId()] = $rule->getPointsAmount();
        }

    	Varien_Profiler::stop("TBT_Rewards:: Predict Newsletter Points");
