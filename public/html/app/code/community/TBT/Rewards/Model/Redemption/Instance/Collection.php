...
//  const POINTS_INST_ID = 'redemption_inst_id';
    ...
    public function hasQuoteItem() {
  	...
  	public function loadData($printQuery = false, $logQuery = false) {
        if ($this->isLoaded()) {
  	    ...
  	    if(!$this->getQuoteItem()) return $this;
    	...
    	foreach($redeemed_points as $key => &$redemption_instance) {
            $ri = Mage::getModel('rewards/redemption_instance')
                ->setData((array)$redemption_instance)
                ->setItem($this->getQuoteItem());
            $this->addItem($ri);
            $num_items++;
        }
    ...
    public function getTotalDiscounts() {
        if(!$this->getQuoteItem() || $this->getSize() == 0) return 0;
    ...
    public function saveToItem($calculate_effect = false) {
        if(!$this->hasQuoteItem()) return $this;
