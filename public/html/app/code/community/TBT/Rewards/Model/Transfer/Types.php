...
{   
    ...
    {
        if($this->getHasLoadedTypeModels()) return $this; //don't load more than once...
    ...
    protected function _defaultTransferBeforeSave(&$transfer) {
        if(!$transfer->getReferenceType() || !$transfer->getReferenceId()) {
            ...
            else if ($transfer->hasData('newsletter_id')) {
            	$transfer->setNewsletterId($transfer->getData('newsletter_id'));
            }
    ...
    protected function _defaultLoadReferenceInformation(&$transfer) {
        ...
        else if ($transfer->isNewsletter()) {
            $transfer->setNewsletterId($transfer->getReferenceId());
        }
