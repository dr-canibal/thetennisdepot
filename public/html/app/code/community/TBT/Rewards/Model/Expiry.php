...
{  
    ...
    public function checkAllCustomers() {
        ...
        foreach($customers as $c) {
            if( !Mage::helper('rewards/expiry')->isEnabled( $c->getStoreId() )  ) continue;
    ...
    public function checkExpirePoints($c) {
        if(!$c->getId()) return $this;
        if( !Mage::helper('rewards/expiry')->isEnabled( $c->getStoreId() )  ) return $this;
        if($c->hasPoints()) {
    ...
    public function getTimeLeftToExpire($c) {
        if(!$c->getId()) return null;
        if( !Mage::helper('rewards/expiry')->isEnabled( $c->getStoreId() )  ) return null;
        if($c->hasPoints()) {
            $last_date_str = $c->getLatestActivityDate();
            if($last_date_str == null) {
                $last_date_str = 0;
    ...
    public function sendWarningEmail($parent, $template, $expires_in_days)
    {
        ...
        $sender  = array(
            'name' => strip_tags(Mage::helper('rewards/expiry')->getSenderName($parent->getStoreId()    )),
            'email' => strip_tags(Mage::helper('rewards/expiry')->getSenderEmail($parent->getStoreId()  ))
        );
