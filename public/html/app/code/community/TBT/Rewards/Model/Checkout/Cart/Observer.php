    ...
    public function checkRedemptions($observer) {
    	...
    	if($this->_getRewardsSess()->isCustomerLoggedIn()) {
	        if($this->_getRewardsSess()->isCartOverspent()) {
	        	$msg = Mage::helper('rewards')->__("You are trying to redeem more points than you have in".
	        										" your balance.  You will not be able to checkout.");
	        	$this->_getCheckoutSession()->addError($msg);
	        }
    	} else {
    		if($this->_getRewardsSess()->hasRedemptions()) {
	        	$msg = Mage::helper('rewards')->__("You are trying to redeem points but you're not logged in.".
	        										"You may not have enough points to checkout.");
	        	$this->_getCheckoutSession()->addError($msg);
    		}
    	}
        return $this;
