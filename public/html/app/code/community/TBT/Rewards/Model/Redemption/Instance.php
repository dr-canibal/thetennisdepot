...
class TBT_Rewards_Model_Redemption_Instance extends Varien_Object
{
    ...
    public function getApplicableRuleInfo() {
        if(!$this->hasApplicableRuleInfo()) {
        	$applicable_rule = Mage::getResourceModel('rewards/catalogrule_rule')->getApplicableReward(
                $this->getDate(), 
                $this->getWebsiteId(), 
    ...
    public function calcEffect() {
		...
		$effect = $this->_help()->amplifyEffect(
			$product_price, 
			$applicable_rule[self::POINTS_EFFECT], 
			$this->getUses()
		);
    ...
    public function getStorableData() {
        $data = $this->getData();
        $keys = array(
            self::POINTS_CURRENCY_ID=>null, 
            self::POINTS_AMT=>null,
            self::POINTS_EFFECT=>null,
            self::POINTS_RULE_ID=>null,
            self::POINTS_APPLICABLE_QTY=>null,
            self::POINTS_USES=>null,
            self::POINTS_INST_ID=>null,
        );
    ...
    public function loadFromRule($rule=null) {
        if($rule != null) $this->setRule($rule);
