...
class TBT_Rewards_Model_Salesrule_Discountmanager extends Mage_Core_Model_Abstract
{
    ...
    public function printDiscounts() {
        echo "All: "; foreach ($this->discount_map as $d) echo implode("|", $d) . ", ";
        echo "\n<BR />";
        echo "Rew: "; foreach ($this->rewards_discount_map as $d) echo implode("|", $d) . ", ";
        echo "  _+_ Spent: "; foreach ($this->rewards_spent_discount_map as $d) echo implode("|", $d) . ", ";
        echo "\n<BR />";
        echo "Reg: "; foreach ($this->regular_discount_map as $d) echo implode("|", $d) . ", ";
        echo "\n<BR />--";
        return $this;
    }    
    ...
    public function getTotalNonSpendingDiscount() {
        $tnsd = 0;
        foreach ($this->rewards_discount_map as $d) { $tnsd += $d['discount']; }
        foreach ($this->regular_discount_map as $d) { $tnsd += $d['discount']; }
        return $tnsd;
    }
}