...
class TBT_Rewards_Customer_SendpointsController extends Mage_Core_Controller_Front_Action
{
    ...
    public function sendAction()
    {
        try {
	    	...
	    	$friend = Mage::getModel('rewards/customer')
	    	           ->setWebsiteId(Mage::app()->getWebsite()->getId())
	    	           ->loadByEmail($friend_email);
	    	$friend_id = $friend->getId();
	    	if (!$friend_id) {
	            Mage::getSingleton('core/session')->addError(
	            	$this->__("There is no customer with that email address (%s).", $friend_email)
	            );
	    	...
	    	if ( !$friend->hasCurrencyId($currency_id) ) {
	    	    ...
	    	    Mage::getSingleton('core/session')->addError(
	    	    	$this->__('%s is not allowed to use the %s points currency.', 
	    	    		$friend->getName() , 
	    	    		$currency_caption
	    	    	)
	    	    );
	        ...
	        if (!$sender->getId()) {
	            Mage::getSingleton('core/session')->addError(
	            	$this->__('You must log in or sign up before sending points to a friend!')
	            );
	        ...
	        if ($points_amt > $balance) {
	            ...
	            Mage::getSingleton('core/session')->addError(
	            	$this->__('You cannot send %s points to %s when you only have %s!', 
		                Mage::getModel('rewards/points')->set($currency_id, $points_amt),
		                $friend->getName(),
		                Mage::getModel('rewards/points')->set($currency_id, $balance)
	                )
	            );
	            $this->_redirect('rewards/customer/');
	            return $this;
			...
			if ($friend_email == $sender->getEmail()) {
				Mage::getSingleton('core/session')->addError(
					$this->__("You cannot send points to yourself!!")
				);
				$this->_redirect('rewards/customer/');
				return $this;
	        }
	        ...
	        if ($is_transfer_successful) {
		    	...
		    	Mage::getSingleton('core/session')->addSuccess(
		    		$this->__('You have successfully sent %s to %s!',
			    	  	$points_sent_string,
			    	  	$friend->getName()
		    	  	)
		    	);
