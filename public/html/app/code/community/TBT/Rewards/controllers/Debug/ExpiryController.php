...
class TBT_Rewards_Debug_ExpiryController extends TBT_Rewards_Debug_AbstractController
{
    ...
    public function sendWarningEmail($parent, $template, $expires_in_days)
    {
        ...
        $sender  = array(
            'name' => strip_tags(Mage::getStoreConfig("trans_email/ident_support/name", $parent->getStoreId())),
            'email' => strip_tags(Mage::getStoreConfig("trans_email/ident_support/email", $parent->getStoreId()))
        );
        ...
        $email->setDesignConfig(array('area'=>'frontend', 'store'=>$parent->getStoreId()))
                ->sendTransactional(
                    ...
                    array(
                        'customer'          => $parent,
                        'store_name'        => $parent->getStore()->getName(),
                        'days_left'         => $expires_in_days,
                        'points_balance'    => $parent->getPointsSummary(),
                    )
                );
...
//        Mage::getSingleton('rewards/special_action');
    ...
    public function secstostr($secs, $d=true, $h=true, $m=true, $s=true) { //@todo translatability?
        ...
        if(($d && $secs>=86400) || (!$h && !$m && !$s )){
            $days=floor($secs/86400);$secs=$secs%86400;$r=$days.' day';if($days<>1){$r.='s';}
            if($secs>0 && ($h || $m || $s )){$r.=', ';}
        }
        if(($h && $secs>=3600) || (!$d && !$m && !$s )){
            $hours=floor($secs/3600);$secs=$secs%3600;$r.=$hours.' hour';if($hours<>1){$r.='s';}
            if($secs>0 && ($d || $m || $s )){$r.=', ';}
            }
        if(($m && $secs>=60) || (!$d && !$h && !$s )){
            $minutes=floor($secs/60);$secs=$secs%60;$r.=$minutes.' minute';if($minutes<>1){$r.='s';}
            if($secs>0 && ($d || $h || $s )){$r.=', ';}
        }
        if($s || (!$d && !$hm && !$m )) {
            $r.=$secs.' second';
