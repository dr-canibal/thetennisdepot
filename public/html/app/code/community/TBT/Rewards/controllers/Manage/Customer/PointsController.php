...
class TBT_Rewards_Manage_Customer_PointsController extends Mage_Adminhtml_Controller_Action
{
    ...
    {
        $this->loadLayout()
            ->_setActiveMenu('rewards/customer_points')
	...
	public function massTransferPointsAction() {
		try {
			...
			foreach($customerIds as $customer_id) {
				try {
					...
					Mage::getSingleton('core/session')->addError(
						$this->__('Failed to transfer points to customer %s', $customer->getName()) . " : ". $e->getMessage()
					);
				}
			}
            
			if($numSuccessfulTransfers > 0) {
				$success = $this->__("Successfully transfered %s to %s customer(s).", 
			...
			Mage::getSingleton('core/session')->addError(
				$this->__('Woops, ran into error while trying to process your request: '). $e->getMessage()
			);
		}
		$this->_redirect('*/*/');
	}
