...
class TBT_Rewards_Manage_CurrencyController extends Mage_Adminhtml_Controller_Action
{
	...
	protected function _initAction() {
		$this->loadLayout()
			->_setActiveMenu('rewards/currency');
	...
	public function editAction() {
		...
		if ($model->getId() || $id == 0) {
			...
			$this->_addContent($this->getLayout()->createBlock('rewards/manage_currency_edit'))
				->_addLeft($this->getLayout()->createBlock('rewards/manage_currency_edit_tabs'));
...
//
//		$this->renderLayout();
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('Can not create currencies. Available in future upgrades of Sweet Tooth.'));
        $this->_redirect('*/*/');
	}
            ...
            try {
                    ...
                    if ($transfer->setStatus($currentStatus, $newStatus) === false) {
                        if($transfer->getStatus() == $newStatus) {
                            $this->_getSession()->addNotice(
                                'Transfer #'. $transferId . ' is already '. $currentStatusCaption
                            );
                        } else {
                            $this->_getSession()->addError(
                                'Transfer #'. $transferId . ' cannot be changed from '. 
                                $currentStatusCaption .' to '. $newStatusCaption
                            );
                        }
                ...
                if($changedTransferCount > 0) {
                    $this->_getSession()->addSuccess($this->__(
                        'Total of %d transfer(s) were successfully changed to '. 
                        $newStatusCaption, $changedTransferCount
                    ));
                }
            } catch (Exception $e) {
