...
class TBT_Rewards_Block_Manage_Promo_Catalog_Edit_Tab_Conditions extends Mage_Adminhtml_Block_Widget_Form
{
    protected function _prepareForm()
    {
        ...
        $renderer->setNewChildUrl(
            $this->getUrl('*/manage_promo_catalog/newConditionHtml/form/rule_conditions_fieldset')
        );
    	...
    	$fieldset->addField('conditions', 'text', array(
            'name' => 'conditions',
            'label' => Mage::helper('catalogrule')->__('Product Conditions'),
            'title' => Mage::helper('catalogrule')->__('Product Conditions'),
            'required' => true,
        ))->setRule($model)->setRenderer(Mage::getBlockSingleton('rule/conditions'));
        
        if($this->_isRedemptionType()) {
        	$fieldset = $form->addFieldset('points_action_fieldset', array(
        		'legend' => Mage::helper('rewards')->__('Customer Spends Points')));
	        ...
	        $fieldset->addField('points_action', 'select', array(
	            'label'     => Mage::helper('salesrule')->__('Customer Spending Style'),
	            'name'      => 'points_action',
	            'options'    => $options, 
