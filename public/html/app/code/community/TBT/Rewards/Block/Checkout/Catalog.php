    ...
    */
    public function formatItemColoumn($_item, $cols_from_right = 2){
        ...
        if($hasRedeemed) {
	        ...
	        $html_points_list .= '<div class="'. self::ITEM_REDEEM_HEADER_CLASS .'" '.
        					 			'onclick="Effect.toggle(\''. $redeem_list_id .'\', \''. self::TOGGLE_EFFECT .'\')" '.
	        							'style="'. self::TOGGLE_BUTTON_STYLE .'" ' .
	        							'title="'. $this->__('Click to see a breakdown of how your points affect this line item.') .'" '.
        					 			'>' . 	$this->__('Points Spent') . 
	        					 '</div> ';
        	$html_points_list .= '<ul 	class=\''. self::ITEM_REDEEM_LIST_CLASS .'\' '.
        						 		'id=\''. $redeem_list_id .'\' style="display:none;">';
...
//        }
    ...
    public function getItemPointsAsList($_item) {
        ...
        $earned_points_list = '<div class="'. self::ITEM_EARN_HEADER_CLASS .'" '.
       					 			'onclick="Effect.toggle(\''. $earn_list_id .'\', \''. self::TOGGLE_EFFECT .'\')" '.
        							'style="'. self::TOGGLE_BUTTON_STYLE .'" ' .
	        						'title="'. $this->__('Click to see a breakdown of how your points affect this line item.') .'" '.
       					 			'>' . 
        					 	$this->__('Points Earned') . 
        					 '</div> ';
    	$currency_points = Mage::helper('rewards/transfer')->getEarnedPointsOnItem($_item);
        $earned_points_list .= '<ul 	class=\''. self::ITEM_EARN_LIST_CLASS .'\' '.
        						 		'id=\''. $earn_list_id .'\' style="display:none;">';
