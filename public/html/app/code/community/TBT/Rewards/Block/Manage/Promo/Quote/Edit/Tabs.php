...
/**
 * Manage Promo Quote Edit Tabs
 *
 * @category   TBT
 * @package    TBT_Rewards
 * @author     WDCA Sweet Tooth Team <contact@wdca.ca>
 */
class TBT_Rewards_Block_Manage_Promo_Quote_Edit_Tabs extends Mage_Adminhtml_Block_Widget_Tabs
{
    ...
    public function __construct()
    {
        ...
        if($this->_isRedemptionType()) {
        	$this->setTitle(Mage::helper('salesrule')->__('Shopping Cart Points Redemption Rule'));
        } else {
       	 	$this->setTitle(Mage::helper('salesrule')->__('Shopping Cart Points Distribution Rule'));
        }
    }
    ...
    protected function _beforeToHtml()
    {
        $this->addTab('main_section', array(
            ...
            'content'   => $this->getLayout()->createBlock('rewards/manage_promo_quote_edit_tab_main')->toHtml(),
            'active'    => true
        ));

        $this->addTab('conditions_section', array(
            'label'     => Mage::helper('salesrule')->__('Conditions'),
            'content'   => $this->getLayout()->createBlock('rewards/manage_promo_quote_edit_tab_conditions')->toHtml(),
        ));

        $this->addTab('actions_section', array(
            'label'     => Mage::helper('salesrule')->__('Actions'),
            'content'   => $this->getLayout()->createBlock('rewards/manage_promo_quote_edit_tab_actions')->toHtml(),
        ));
    ...
    protected function _isRedemptionType() {
    	...
    	Mage::getSingleton('rewards/session')->addError(
    		...
    		"Promo/Edit/Tab/Actions so assumed redemption.");
    	return true;
    }
    /**
    ...
    protected function _isDistributionType() {
    	...
    	Mage::getSingleton('rewards/session')->addError(
    		"Could not determine rule type in ".
    		"Promo/Edit/Tab/Actions so assumed distribution.");
    	return true;
    }
