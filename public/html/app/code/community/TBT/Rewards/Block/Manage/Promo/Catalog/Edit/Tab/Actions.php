...
class TBT_Rewards_Block_Manage_Promo_Catalog_Edit_Tab_Actions extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    protected function _prepareForm()
    {
        ...
        if($this->_isDistributionType()) {
	        ...
	        $fieldset->addField('points_amount', 'text', array(
	            ...
	            'label' => Mage::helper('rewards')->__('Points Amount (X)')
	        ));
	        
	        $fieldset->addField('points_amount_step', 'text', array(
	            'name' => 'points_amount_step',
	            'label' => Mage::helper('rewards')->__('Monetary Step (Y)'),
	        ));
	...
	//        ));
	        ...
	        $fieldset->addField('points_max_qty', 'text', array(
	            'name' => 'points_max_qty',
	            'label' => Mage::helper('rewards')->__('Maximum Total of Points To Transfer (0 for unlimited)'),
	        ));

	        $simple_actions = array(
                '' => Mage::helper('salesrule')->__('No Discount -- '),
        ...
        } else { // NOT distribution rule (ie, this is a catalog rule)
        ...
        $fieldset->addField('points_catalogrule_simple_action', 'select', array(
            'label'     => Mage::helper('salesrule')->__('Apply'),
            'name'      => 'points_catalogrule_simple_action',
            'options'    => $simple_actions,
		...
		if($this->_isRedemptionType()) {
	        $fieldset->addField('points_uses_per_product', 'text', array(
	            'name' => 'points_uses_per_product',
	            'required' => true,
	            'class' => 'validate-not-negative-number',
	        	'value'	=>	1,
	            'label' => Mage::helper('rewards')->__('Uses Allowed Per Product (0 for unlimted)'),
	        ));
		}

        $form->setValues($model->getData());
        $fieldset->addField('simple_action', 'hidden', array(
            'name'      => 'simple_action',
            'value'    => 'by_fixed',
        ));

        $fieldset->addField('discount_amount', 'hidden', array(
        ...
        $fieldset->addField('stop_rules_processing', 'hidden', array(
            'label'     => Mage::helper('salesrule')->__('Stop further rules processing'),
            'title'     => Mage::helper('salesrule')->__('Stop further rules processing'),
    ...
    private function _isRedemptionType() {
    	...
    	Mage::getSingleton('adminhtml/session')->addError(
    		...
    		"Catalog/Edit/Tab/Actions so assumed redemption.");
    	return true;
    }
    ...
    private function _isDistributionType() {
    	...
    	Mage::getSingleton('adminhtml/session')->addError(
    		"Could not determine rule type in ".
    		"Catalog/Edit/Tab/Actions so assumed distribution.");
    	return true;
    }
