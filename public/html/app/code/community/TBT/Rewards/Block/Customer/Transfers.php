 ...
 */
class TBT_Rewards_Block_Customer_Transfers extends Mage_Core_Block_Template
{
    protected $default_cell_renderer = "rewards/customer_transfers_reference";
	...
	protected function _construct()
	{
		...
		$this->headerText = $this->__("Transfers");
		$this->setTemplate('rewards/customer/transfers.phtml');
	}
    ...
    public function getReferenceCellHtml($transfer) {
        if(!$this->hasData("referrence_cell_renderers")) {
            $referrence_cell_renderers = Mage::getSingleton('rewards/transfer_types')->getTRefCellRenderers();
            $this->setData('referrence_cell_renderers', $referrence_cell_renderers);
        }
        $renderers = $this->getData("referrence_cell_renderers");
        if(isset($renderers[$transfer->getReferenceType()])) {
            $renderer = $renderers[$transfer->getReferenceType()];
        } else {
            $renderer = $this->default_cell_renderer;
        }
        try {
            $renderer = Mage::getBlockSingleton($renderer);
        } catch (Exception $e) {
            $renderer = Mage::getBlockSingleton($this->default_cell_renderer); // if using the custom renderer fails, use the default one.
        }
        if($renderer instanceof Varien_Object) {
            $renderer->setTransfer($transfer);
            $renderer->setCustomer($this->getCustomer());
        }
    ...
    public function _getTransferSummary(){
    	...
    	->addFilter("customer_id", $cust->getId())
			->selectPointsCaption('points_caption')
            ->setOrder('creation_ts', 'DESC');
    	return $transfers;
    }
    ...
    public function getAssociatedFriend($transferId)
    {
        ...
        if ($ref->getReferenceType == TBT_Rewards_Model_Transfer_Reference::REFERENCE_FROM_FRIEND
            || $ref->getReferenceType() == TBT_Rewards_Model_Transfer_Reference::REFERENCE_TO_FRIEND) {
                return $ref->getReferenceId();
        }
        else {
            return null;
