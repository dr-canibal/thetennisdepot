...
class TBT_Rewards_Block_Checkout_Cart_Item_Renderer extends Mage_Checkout_Block_Cart_Item_Renderer
{
	...
	public function getRedemptionData() {
        ...
        foreach ($redeemed_points as $point) {
            ...
            $adjusted_price = Mage::helper('rewards')->priceAdjuster($_item->getBaseCalculationPrice(), $discount, false);
            if($adjusted_price < 0) {
                $adjusted_price = 0;
            }
                        
            $discount = ($_item->getBaseCalculationPrice() - $adjusted_price ) * $points_applic_qty;
