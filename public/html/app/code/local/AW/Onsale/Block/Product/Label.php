...
class AW_Onsale_Block_Product_Label extends Mage_Catalog_Block_Product_Abstract
{
	...
	public function setProduct($product)
	{
        ...
        if (!$product->getCreatedAt()){
            $product->setCreatedAt(Mage::helper('onsale')->getCustomAttributeValue('created_at', $product->getId()));
        }
        # End fix #2111
        ...
        $overNewIsSettedUp = ($product->getNewsFromDate() || $product->getNewsToDate());
      
        # Fixed Wrong "Overrides Native New Option" behavior
      	if ( ($isNew && !$overNewIsSettedUp) || ($isNativeNew && !$overridesNativeNew && $overNewIsSettedUp) ){
			if ( Mage::helper('onsale')
