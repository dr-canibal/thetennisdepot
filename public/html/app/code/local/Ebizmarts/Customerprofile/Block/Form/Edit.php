...
class Ebizmarts_Customerprofile_Block_Form_Edit extends Mage_Customer_Block_Form_Edit {
    ...
    public function getAvatar($customerId, $w = 120, $h = 120)
    {
		...
		if ( $name != '' ) {
			...
			if ( $resize_object->resize_limitwh($w, $h, $small_name) === false ) {
				...
				return '<img src="'.str_replace($base_dir, '', $small_name).'" alt="' . $customer->getNickname() . '" />';
			}
		} else {
			return '';
		}
