...
//       die("Earned [{$earned_points_string}], Spent [{$redeemed_points_string}], order earned points total: [". print_r($order->getTotalEarnedPoints(), true) . "], order redeemed points total: [" . print_r($order->getTotalSpentPoints(), true) . "]");
        	
        	if ($order->hasPointsEarning()) {
        		if($this->_getRewardsSession()->isAdminMode()) {
	                Mage::getSingleton('core/session')->addSuccess( 
	                	Mage::helper('rewards')->__('The customer was credited %s for the order.', $earned_points_string)
	                );
        		} else {
	                Mage::getSingleton('core/session')->addSuccess( 
	                	Mage::helper('rewards')->__('You earned %s on the order you just placed.', $earned_points_string)
	                );
        		}
        	}
        	if ($order->hasPointsSpending()) {
        		if($this->_getRewardsSession()->isAdminMode()) {
	        		Mage::getSingleton('core/session')->addSuccess( 
	                	Mage::helper('rewards')->__('The customer was deducted %s for the order.', $redeemed_points_string)
	                );
        		} else {
	        		Mage::getSingleton('core/session')->addSuccess( 
	                	Mage::helper('rewards')->__('You spent %s on the order you just placed.', $redeemed_points_string)
	                );
        		}
        	}
        	if ( $order->hasPointsEarningOrSpending() ) {
        		if($this->_getRewardsSession()->isAdminMode()) {
	        		Mage::getSingleton('core/session')
	                    ->addNotice( Mage::helper('rewards')->__("The customer's point transactions may be in the 'pending' status.") );	
        		} else {
	        		Mage::getSingleton('core/session')
	                    ->addNotice( Mage::helper('rewards')->__('Your point transactions are currently pending and will be approved when we finish processing your order.') );	
        		}
        	}
