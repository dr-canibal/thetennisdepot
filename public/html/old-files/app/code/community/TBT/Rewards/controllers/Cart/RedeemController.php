...
class TBT_Rewards_Cart_RedeemController extends Mage_Core_Controller_Front_Action
{
    public function catalogAction()
    {
        ...
        $quote->setCartRedemptions($cart_redemptions)
            ->save();
    }
		...
		$customer = Mage::getSingleton('rewards/session')->getSessionCustomer();
            ...
            foreach($rule_list as $rule_id){
                ...
                if(array_search((int)$rule_id, explode(',', $quote->getCartRedemptions())) === false){
                    $this->_getSess()->addError($this->__("The rule " . $rule->getName() . " does not apply to your cart."));
                }else{
                    ...
                    if($customer_point_balance[$rule->getPointsCurrencyId()] < $rule->getPointsAmount()){
                        $this->_getSess()->addError(
                            $this->__("You do have have enough %s Points.", $currency_captions[$rule->getPointsCurrencyId()]). 
                        ...
                        if(array_search($rule_id, $applied_redemptions) === false){ // if the rule is not already listed
                            if (array_search($rule_id, $applicable_redemptions) !== false) {
                            	$key = array_search($rule_id, $applicable_redemptions);
                            	unset($applicable_redemptions[$key]);
                            	$applied_redemptions[] = $rule_id;
                            	//$applied_redemptions = array_unique($applied_redemptions);
                            	$quote->setCartRedemptions(implode(',', $applicable_redemptions))
                                    ->setAppliedRedemptions(implode(',', $applied_redemptions));
                                $doSave = true;
                            }
                        } else {
                            $this->_getSess()->addError(
                                $this->__("The rule entitled '%s' is already applied to your cart.", $rule->getName())
                            );
                        }
                    }
                }
...
//    protected function enableRedemption($rule_id) {
//        $quote = $this->getQuote();
//        $applied_redemptions = explode(',', $quote->getAppliedRedemptions());
//        $applicable_redemptions = explode(',', $quote->getCartRedemptions());
//        $doSave = false;
...
//    
        ...
        try {
            ...
            foreach($rule_list as $rule_id){
                ...
                if(array_search((int)$rule_id, $applied_redemptions) === false){ 
                    $this->_getSess()->addError($this->__("The rule " . $rule->getName() . " was not applied to your cart."));
                }else{
                    // index at which the possibly removable rule id was found.
                    $foundRuleIdIndex = array_search($rule_id, $applied_redemptions);   
                    if($foundRuleIdIndex === false){
                        $this->_getSess()->addError(
                            $this->__("The rule entitled '%s' is not applied to your cart.", $rule->getName())
                        );
                        $flag = false;  
                    }else{
                        unset($applied_redemptions[$foundRuleIdIndex]);
                        $applicable_redemptions[] = $rule_id;
                        $quote->setAppliedRedemptions(implode(',', $applied_redemptions))
                            ->setCartRedemptions(implode(',', $applicable_redemptions));
                        $doSave = true; 
                    }
                } 
            }       
    ...
    public function catalogremoveAction(){
        ...
        $item = Mage::getSingleton('checkout/cart')
                    ->getQuote()->getItemById($item_id);
        if (!$item->getId()) {
        	Mage::getSingleton('customer/session')->addError($this->__("An item was not selected or the item selected was invalid"));
        	return;
        }
  ...
  /*  
   * This was used for points spending operations
    public function setPointsSpendingAction() {
    	$rule_id = $this->getRequest()->get('sr_id');
        $amount = $this->getRequest()->get('amount');
        $currency = 1;//$this->getRequest()->get('curr');
