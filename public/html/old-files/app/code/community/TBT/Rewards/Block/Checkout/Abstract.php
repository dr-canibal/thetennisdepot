...
class TBT_Rewards_Block_Checkout_Abstract extends Mage_Checkout_Block_Cart
{
    ...
    public function genRuleCtrlImg($rule_id, $is_add = true, $is_cart = true, $item_id = 0, 
        ...
        if(!empty($redemption_instance_id)) $params['inst_id'] = $redemption_instance_id;
        if (!$is_cart) {
            $params['item_id'] = $item_id;
        }
        $url = $this->getUrl($url_key, $params);
        
        if(empty($callback)) $callback = "true";
...
EOT;
     ...
     * TODO why does this take a quote?  Couldn't you just do $this->getQuote()?
     *
     * @param unknown_type $quote
     * @return array
     */
    public function collectShoppingCartRedemptions($quote=null) {
    	if($quote == null) $quote = $this->getQuote();
