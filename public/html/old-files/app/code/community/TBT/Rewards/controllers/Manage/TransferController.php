 ...
 */
class TBT_Rewards_Manage_TransferController extends Mage_Adminhtml_Controller_action
{
	const EXPORT_FILE_NAME = 'point_transfers';
	
	protected function _initAction() {
		$this->loadLayout()
			->_setActiveMenu('rewards/transfer');
	...
	public function editAction() {
		...
		if ($model->getId() || $id == 0) {
			...
			$this->_addContent($this->getLayout()->createBlock('rewards/manage_transfer_edit'))
				->_addLeft($this->getLayout()->createBlock('rewards/manage_transfer_edit_tabs'));
	...
	public function newAction() {
		...
		$this->_addContent(
            $this->getLayout()->createBlock('rewards/manage_transfer_edit')
        )->_addLeft(
            $this->getLayout()->createBlock('rewards/manage_transfer_edit_tabs')
        );
	...
	public function revokeAction() {
        ...
        try {
	        $new_transfer_id = Mage::helper('rewards/transfer')->createRevokedTransfer(
	            $original_transfer->getQuantity() * -1,
	            $original_transfer->getCurrencyId(),
	            $original_transfer_id,
	            $original_transfer->getCustomerId()
	        );
        } catch (Exception $ex) {
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('adminhtml')->__(
                        'Total of %d record(s) were successfully deleted', count($transferIds)
                    )
                );
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
            ...
            try {
                    ...
                    if ($transfer->setStatus($currentStatus, $newStatus) === false) {
                        if($transfer->getStatus() == $newStatus) {
                            $this->_getSession()->addNotice(
                            	$this->__('Transfer #%s is already %s.', $transferId, $currentStatusCaption)
                            );
                        } else {
                ...
                if($changedTransferCount > 0) {
                    $this->_getSession()->addSuccess($this->__(
                        'A total of %s transfer(s) were successfully changed to %s.', $changedTransferCount, $newStatusCaption
                    ));
                }
            } catch (Exception $e) {
    ...
    public function ordersGridAction()
    {
        ...
        $this->getResponse()->setBody($this->getLayout()
            ->createBlock('rewards/manage_transfer_edit_tab_grid_orders')->toHtml());
    }
    ...
    public function pollsGridAction()
    {
    	...
    	$this->gridModelInit();
        $this->getResponse()->setBody($this->getLayout()
            ->createBlock('rewards/manage_transfer_edit_tab_grid_polls')->toHtml());
    }
    public function friendsGridAction()
    {
    	...
    	$this->gridModelInit();
        $this->getResponse()->setBody($this->getLayout()
            ->createBlock('rewards/manage_transfer_edit_tab_grid_friends')->toHtml());
    }
    ...
    public function reviewsGridAction()
    {
    	$this->gridModelInit();
        $this->getResponse()->setBody($this->getLayout()
            ->createBlock('rewards/manage_transfer_edit_tab_grid_reviews')->toHtml());
    }
    ...
    public function transfersGridAction()
    {
    	$this->gridModelInit();
        $this->getResponse()->setBody($this->getLayout()
            ->createBlock('rewards/manage_transfer_edit_tab_grid_transfers')->toHtml());
    }
    ...
    public function tagsGridAction()
    {
    	$this->gridModelInit();
        $this->getResponse()->setBody($this->getLayout()
            ->createBlock('rewards/manage_transfer_edit_tab_grid_tags')->toHtml());
    }
