        ...
        if ($rule->getId()) {
            if ($crActions->isGivePointsAction($rule->getPointsAction())) {
                ...
                foreach($order_items as &$item) {
                   // Mage::helper('rewards/debug')->dd($rule->getName(), 0, 0)->dd($item->getName(), false, false);
                }
            }
                ...
                $points_to_transfer = max($max_points, (int)$rule->getPointsSpending());
                if ($rule->getPointsMaxQty() > 0) {
                    if ($points_to_transfer > $rule->getPointsMaxQty()) {
                        $points_to_transfer = $rule->getPointsMaxQty();
                    }
   ...
   /**
    ...
    public function collectShoppingCartRedemptions($cart=null)
    {
        ...
        foreach (explode(',', $cart->getAppliedRedemptions()) as $rule_id) {
            ...
            $points = $this->calculateCartPoints($rule->getId(), $cart->getAllItems(), true);
            $val = $points;
            $val['name'] = $rule->getName();
            
            $applied_redemptions[$rule_id] = $val; 
        }
        ...
        foreach ($cart_redemptions as $rule_id => $is_applicable) {
            ...
            $val = $points;
            $val['name'] = $rule->getName();
            $applicable_redemptions[$rule_id] = $val;
        }
        
        return array('applied'=>$applied_redemptions, 'applicable'=>$applicable_redemptions);
    }
