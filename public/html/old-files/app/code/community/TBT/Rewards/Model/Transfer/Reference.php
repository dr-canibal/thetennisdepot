...
{   
    ...
    const REFERENCE_REVIEW      = 2;
    const REFERENCE_POLL        = 3;
    const REFERENCE_TAG         = 4;
    ...
    {
        return ($typeId == TBT_Rewards_Model_Transfer_Reference::REFERENCE_REVIEW);
    }
    ...
    {
        return ($typeId == TBT_Rewards_Model_Transfer_Reference::REFERENCE_NEWSLETTER);
    }
    ...
    {
        return ($typeId == TBT_Rewards_Model_Transfer_Reference::REFERENCE_TAG);
    }
    ...
    public function getOptionArray()
    {
        return array(
            ...
            self::REFERENCE_REVIEW      => Mage::helper('rewards')->__('Review'),
            self::REFERENCE_POLL        => Mage::helper('rewards')->__('Poll'),
            self::REFERENCE_TAG         => Mage::helper('rewards')->__('Tag'),
            ...
            self::REFERENCE_NEWSLETTER  => Mage::helper('rewards')->__('Newsletter')
        );
    }
    ...
    public function getReferenceCaption($id) {
        ...
        if ( ($id >= self::REFERENCE_ORDER) && ($id <= self::REFERENCE_NEWSLETTER) ) {
            return $reference[$id];
        }
        else {
            return null;
    ...
    public function loadReferenceByTransferId($transfer_id) {
        $read = Mage::getSingleton('core/resource')->getConnection('core_read');
        $select = $read->select()
            ->from($this->_getResource()->getTable('transfer_reference'))
            ->where('rewards_transfer_id = ?', $transfer_id);
