...
class TBT_Rewards_Block_Manage_Transfer_Edit extends Mage_Adminhtml_Block_Widget_Form_Container
{
    ...
    public function __construct()
    {
        ...
        if( $this->_getTransfer() && $this->_getTransfer()->getId() ) {
            $this->_addButton('create_new', array(
                'label'     => Mage::helper('adminhtml')->__('Create A New Transfer'),
                'onclick'   => 'window.location=\''.$this->getUrl('*/*/new', array('back' => $this->_backController )).'\'',
            ), -100);
        } else {
        ...
        if ( $this->_getTransfer() && $this->_getTransfer()->getId()
            && ($this->_getTransfer()->getReasonId() != TBT_Rewards_Model_Transfer_Reason::REASON_SYSTEM_REVOKED) ) {
            	
		        $this->_addButton('revoke', array(
		            'label'     => Mage::helper('rewards')->__('Revoke Transfer'),
		            'onclick'   => 'revokeTransfer()'
		        ), -100);
        }
    ...
    {
    	if ($params === null) {
    		$url = $this->getUrl( $this->_backModule . '/'. 
    						$this->_backController .'/' . $this->_backAction );
    	} else if (isset($params['customer_id'])){
    		if ($this->_backController != 'manage_customer_points') {
    			$url = $this->getUrl( $this->_backModule . '/'. 
    						$this->_backController .'/' . $this->_backAction, Array(
    		...
    		$url = $this->getUrl( $this->_backModule . '/'. 
    						$this->_backController .'/' . $this->_backAction, $params );
    	}
        return $url;
    }
}