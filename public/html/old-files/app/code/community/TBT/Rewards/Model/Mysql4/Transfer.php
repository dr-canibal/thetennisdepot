...
class TBT_Rewards_Model_Mysql4_Transfer extends Mage_Core_Model_Mysql4_Abstract
{
    ...
    protected function _afterLoad(Mage_Core_Model_Abstract $object)
    {
		...
		$select = $this->_getReadAdapter()->select()
            ->from($this->getTable('transfer_reference'))
            ->where('rewards_transfer_id = ?', $object->getId());
    ...
    {		
        $this->getSelect()->join(
            array('cps' => 'customer_entity'),
            $this->getMainTable().'.customer_id = cps.entity_id'
        );
        $this->getSelect()->join(
            array('cpsv' => 'customer_entity_varchar'),
            'cps.entity_id = cpsv.entity_id AND cpsv.attribute_id = 1'
        );
