...
class TBT_Rewards_Model_Mysql4_CatalogRule_Rule_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract
{
    ...
    public function _initSelect() {
        ...
        parent::_initSelect();
        $this->getSelect()->distinct(true)->joinInner(
            array('product_table' => $this->getTable('catalogrule/rule_product')),
            'main_table.rule_id = product_table.rule_id',
            array(
...
//                'product_id' =>  "product_table.product_id"
        ...
        )->where('from_time=0 or from_time<=? or to_time=0 or to_time>=?', now())
    ...
    public function getRedemRules() {
    	$this->getSelect()->where('main_table.points_action LIKE \'%deduct%\' '
    							. 'or main_table.points_action LIKE \'%discount%\'');
    }
