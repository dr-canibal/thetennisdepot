...
class TBT_Rewards_Model_Mysql4_Transfer_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract
{
    ...
    public function addRules() {
        $alias = 'rule_name';   
        $this->getSelect()->joinLeft(
            array('salesrules' => $this->getTable('salesrule/rule')),
            'reference_table.rule_id = salesrules.rule_id',
            array(
                'rule_id' =>   "reference_table.rule_id",
                'salesrule_name' =>   "salesrules.name",
            )
        );
        $this->getSelect()->joinLeft(
            array( 'catalogrules' => $this->getTable('catalogrule/rule')),
            'reference_table.rule_id = catalogrules.rule_id',
            array(
                'catalogrule_name' =>   "catalogrules.name",
            )
        );     
    ...
    public function selectCurrency()
    {
        if(!$this->didSelectCurrency) {
            $this->getSelect()->joinLeft(
                    ...
                    'currency_table.rewards_currency_id=main_table.currency_id',
                    array('currency'=>'caption')
            );
...
//      }
    ...
    public function addFullNameFilter($store)
    {
		if (!Mage::app()->isSingleStoreMode()) {
			...
			->where('store_currency_table.store_id in (?)', array(0, $store));
    ...
    public function selectOnlyDistributions() {
        ...
        $this->getSelect()->where('main_table.reason_id IN (?)', array(0, $reasons))
        		->order('main_table.creation_ts DESC');
