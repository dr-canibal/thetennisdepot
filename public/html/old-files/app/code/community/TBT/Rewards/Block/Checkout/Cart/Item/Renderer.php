...
class TBT_Rewards_Block_Checkout_Cart_Item_Renderer extends Mage_Checkout_Block_Cart_Item_Renderer
{
	...
	public function getRedeemedPoints() {
		if($this->redeemed_points == null) {
        	$this->redeemed_points = Mage::helper('rewards')->unhashIt(
        		$this->getItem()->getRedeemedPointsHash()
        	);
		}
    ...
    public function getRowTotalBeforeRedemptions() {
        if ($this->helper('tax')->priceIncludesTax() ) {
  	        $price = $this->getItem()->getRowTotalBeforeRedemptions() + $this->getItem()->getTaxAmount();
    ...
    public function getEarningData() {
        ...
        foreach ($earned_points as $cid => $points_qty) {
    		$earned_points_str = Mage::helper('rewards')->getPointsString(array($cid => $points_qty));
	 ...
	 */	
    public function genRuleCtrlImg($rule_id, $is_add = true, $is_cart = true, $item_id = 0, 
    	$redemption_instance_id = 0, $callback="true") {
    	$img_block_name = "rulectrlimg-$rule_id-$item_id";
    	$img_block_class = 'rewards/checkout_cart_rulectrlimg';
    	$img_html = $this->getBlockHtml($img_block_name);
    	if(empty($img_html)) {
	        $img_block = $this->getLayout()->createBlock($img_block_class, $img_block_name);
	        $img_block->init($rule_id, $is_add, $is_cart, $item_id, $redemption_instance_id, $callback);
	    	$img_html = $img_block->toHtml();
    	}
