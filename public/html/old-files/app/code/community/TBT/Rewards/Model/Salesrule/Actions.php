...
{   
    ...
    {
        $options = array_merge(
        	array('' => Mage::helper('rewards')->__('--Don\'t Transfer Points--')), 
        	$this->getDistributionsOptionArray(), 
        	array('' => Mage::helper('rewards')->__('--  --')),
        	$this->getRedemptionsOptionArray()
        );
        return $options;
    }
    ...
    {
        return array(
            self::ACTION_DEDUCT_POINTS => Mage::helper('rewards')->__('Deduct X points from customer'),
            self::ACTION_DEDUCT_BY_AMOUNT_SPENT => Mage::helper('rewards')->__('For every Y amount spent, deduct X points'),
            self::ACTION_DEDUCT_BY_QTY => Mage::helper('rewards')->__('For every Z qty purchased, deduct X points'),
            //self::ACTION_DISCOUNT_BY_POINTS_SPENT => Mage::helper('rewards')->__(' For every X points spent, discount Y from price'),
    ...
    {
        return array(
            self::ACTION_GIVE_POINTS => Mage::helper('rewards')->__('Give X points to customer'),
            self::ACTION_GIVE_BY_AMOUNT_SPENT => Mage::helper('rewards')->__('For every Y amount spent, give X points'),
            self::ACTION_GIVE_BY_QTY => Mage::helper('rewards')->__('For every Z qty purchased, give X points'),
    ...
    public function getDistributionActions() {
    	return array(
    		self::ACTION_GIVE_POINTS, 
    		self::ACTION_GIVE_BY_AMOUNT_SPENT, 
    		self::ACTION_GIVE_BY_QTY,
    	);
    }
    
    public function getRedemptionActions() {
    	return array(
    		self::ACTION_DEDUCT_POINTS, 
    		self::ACTION_DEDUCT_BY_AMOUNT_SPENT, 
    		self::ACTION_DEDUCT_BY_QTY, 
    		self::ACTION_DISCOUNT_BY_POINTS_SPENT,
    	);
    }
    ...
    public function isRedemptionAction($action) {
        return      $this->isDeductPointsAction($action) 
                ||  $this->isDeductByAmountSpentAction($action) 
                ||  $this->isDeductByQtyAction($action) 
                ||  $this->isDiscountByPointsSpentAction($action);
    }
    /**
    ...
    public function isDistributionAction($action) {
        return      $this->isGivePointsAction($action) 
                ||  $this->isGiveByQtyAction($action)
                ||  $this->isGiveByAmountSpentAction($action);
    }
