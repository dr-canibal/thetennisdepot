...
class TBT_Rewards_Model_Sales_Quote_Address_Total_Rewards extends Mage_Sales_Model_Quote_Address_Total_Abstract
{
    ...
    {     	
        ...
        $address->setSubtotal($address->getSubtotal() + $this->getFinalPrice($address) );
        $address->setBaseSubtotal($address->getBaseSubtotal() + $this->getFinalPrice($address));
    ...
    public function getFinalPrice(Mage_Sales_Model_Quote_Address $address) {
        ...
        if($acc_diff < 0.0001 && $acc_diff > -0.0001) {
            $acc_diff = 0;
        }
