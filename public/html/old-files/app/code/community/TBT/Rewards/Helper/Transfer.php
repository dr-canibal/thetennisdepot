...
class TBT_Rewards_Helper_Transfer extends Mage_Core_Helper_Abstract
{
    ...
    public function transferOrderPoints($num_points, $currency_id, $order_id, $rule_id)
    {
        ...
        $transfer->setOrderId($order_id)
            ->setCustomerId( Mage::getModel('sales/order')->load($order_id)->getCustomerId() )
            ->save();
    ...
    public function transferReviewPoints($num_points, $currency_id, $review_id, $rule_id)
    {
        ...
        $transfer->setReviewId($review_id)
            ...
            ->setCustomerId( Mage::getModel('review/review')->load($review_id)->getCustomerId() )
            ->save();
    ...
    public function transferSendfriendPoints($num_points, $currency_id, $rule_id)
    {
        ...
        $transfer
            ->setComments(Mage::getStoreConfig('rewards/transferComments/tellAFriendEarned'))
    ...
    public function transferPollPoints($num_points, $currency_id, $poll_id, $rule_id)
    {
        ...
        $transfer->setPollId($poll_id)
            ->setComments(Mage::getStoreConfig('rewards/transferComments/pollEarned'))
            ->setCustomerId( Mage::getSingleton('customer/session')->getCustomerId() )
            ->save();
    ...
    public function createRevokedTransfer($num_points, $currency_id, $reference_transfer_id, $customer_id)
    {
        ...
        $transfer->setId(null)
            ...
            ->setQuantity($num_points)
            ->setCustomerId($customer_id)
            ->setReferenceTransferId($reference_transfer_id)
            ->setReasonId(TBT_Rewards_Model_Transfer_Reason::REASON_SYSTEM_REVOKED)
            ->setComments(Mage::getStoreConfig('rewards/transferComments/revoked'), $original_transfer->getComments())
            ->save();
...
/**
    ...
    public function transferPointsToFriend($num_points, $currency_id, $friend_id, $personal_comment)
    {
        ...
        $sender_comments = $this->__(
        	Mage::getStoreConfig('rewards/transferComments/sendToFriend'), 
        	$to_customer->getName(), 
        	$personal_comment
        );
        $sender_transfer->setId(null)
            ->setCurrencyId($currency_id)
            ->setQuantity($num_points * -1)
            ->setCustomerId($from_customer->getId())
            ->setToFriendId($to_customer->getId())
            ->setComments($sender_comments)
            ->save();
        
        $receiver_comment = $this->__(
        	Mage::getStoreConfig('rewards/transferComments/receiveFromFriend'), 
        	$from_customer->getName(), 
        	$personal_comment
        );
        $recipient_transfer->setId(null)
            ->setCurrencyId($currency_id)
            ->setQuantity($num_points)
            ->setCustomerId($to_customer->getId())
            ->setFromFriendId($from_customer->getId())
            ->setComments($receiver_comment)
            ->save();
     ...
     */
    public function getCatalogRewardsRuleIds($item)
    {
        return $this->getCatalogRewardsRuleIdsForProduct($item->getProductId());
    }
     ...
     * @return  array(int)                          : An array of rule ID's that are associated with the item
     */
    public function getCatalogRewardsRuleIdsForProduct($productId)
    {
    	/* TODO: make this method return REWARDS-SYSTEM rule id's ONLY */
        // look up all rule objects associated with this item
        $now = now();
                
        $rule_data = Mage::getResourceModel('catalogrule/rule')
            ->getRuleProductsForDateRange($now, $now, $productId);
            //getRulesForProduct($date, $wId, $pId)
            
        //$this->formatDate($date, false)
        ...
        foreach ($rule_data as $rule) {
        	//if ($rule['points_action']) {
                if ( ($rule['from_time'] != 0) && (strtotime($now) < $rule['from_time']) ) {
                	continue;
                }
                else if ( ($rule['to_time'] != 0) && (strtotime($now) > $rule['to_time']) ) {
                	continue;
                }
                else {
    ...
    /**
     * TODO: Wow this needs to DEFITELY move to a rewards item model.
     *
            ...
            foreach($points_to_earn as $points) {
                if ($points) {
                    ...
                    if (isset($currency_points[$points[TBT_Rewards_Model_Catalogrule_Rule::POINTS_CURRENCY_ID]])) {
                        $currency_points[$points[TBT_Rewards_Model_Catalogrule_Rule::POINTS_CURRENCY_ID]] +=
                           ($points[TBT_Rewards_Model_Catalogrule_Rule::POINTS_AMT]
                                * $points[TBT_Rewards_Model_Catalogrule_Rule::POINTS_APPLICABLE_QTY]);
                    }
                    else {
                        $currency_points[$points[TBT_Rewards_Model_Catalogrule_Rule::POINTS_CURRENCY_ID]] =
                           ($points[TBT_Rewards_Model_Catalogrule_Rule::POINTS_AMT]
    ...
    {
    	/* TODO WDCA IMMEDIATE - take cart redemptions out of calculation
         *      add catalog redemptions to calculation using MODIFIED calculateCatalogDiscounts()
         */
    	$discount = 0.0;

//        $cart_redemptions = explode(',', $cart->getCartRedemptions());
//        foreach ($cart_redemptions as $rule_id) {
//            $rule = Mage::getModel('salesrule/rule')->load($rule_id);
//            if (!$rule->getId()) {
//                continue;
//            }
//            
//            $discount += $this->calculateCartDiscounts($rule_id, $cart->getAllItems());
//        }
	...
	private function isItem($obj) {
   		$ret = false;
		if($obj instanceof Mage_Sales_Model_Quote_Item
				|| $obj instanceof Mage_Sales_Model_Quote_Item_Abstract  
				|| $obj instanceof Mage_Sales_Model_Quote_Address_Item 
				|| $obj instanceof Mage_Sales_Model_Order_Item
				|| $obj instanceof Mage_Sales_Model_Order_Invoice_Item
				|| $obj instanceof Mage_Sales_Model_Order_Creditmemo_Item 
				|| $obj instanceof Mage_Sales_Model_Order_Shipment_Item ) { // params are function($rule)
			$ret = true;
		}
		return $ret;
	}
    ...
    public function calculateCatalogPoints($rule_id, $item, $allow_redemptions)
    {
        ...
        if($this->isItem($item) ) {
        	...
        	$profit = $item->getRowTotal() - ($product_cost * $item->getQty());
        } else {
...
//        	echo("price is $price, profit is $profit, rule is {$rule->getPointsAction()} ".
//        			"with name {$rule->getName()}. ".
//        			"<BR />".
//        			"Points to transfer is $points_to_transfer".
//        			"<BR /><BR /><BR />");
     ...
     */
    public function getTotalAssociatedItemPrice($order_items, $required_id)
    {
        $price = 0;
