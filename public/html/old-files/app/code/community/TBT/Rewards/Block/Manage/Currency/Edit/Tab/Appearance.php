...
class TBT_Rewards_Block_Manage_Currency_Edit_Tab_Appearance extends Mage_Adminhtml_Block_Widget_Form
{
	...
	protected function _prepareForm()
	{
        ...
        $fieldset->addField('image', 'text', array(
            'name' => 'image',
            'label' => Mage::helper('rewards')->__('Image Path (relative to store skin)'),
            'title' => Mage::helper('rewards')->__('Image Path (relative to store skin)'),
        ));
        
        $fieldset->addField('image_width', 'text', array(
            'name' => 'image_width',
            'class'     => 'validate-number',
            'label' => Mage::helper('rewards')->__('Image Width (px)'),
            'title' => Mage::helper('rewards')->__('Image Width (px)'),
        ));

        $fieldset->addField('image_height', 'text', array(
            'name' => 'image_height',
            'class'     => 'validate-number',
            'label' => Mage::helper('rewards')->__('Image Height (px)'),
            'title' => Mage::helper('rewards')->__('Image Height (px)'),
        ));
         ...
         */
        $fieldset->addField('image_write_quantity', 'select', array(
            'label'     => Mage::helper('rewards')->__('Print Points on Image'),
            'title'     => Mage::helper('rewards')->__('Print Points on Image'),
            'name'      => 'image_write_quantity',
            'options'    => array( 0=>'No', 1=>'Yes' ),
        ));
        
        $fieldset->addField('font', 'text', array(
            'name' => 'font',
            'label' => Mage::helper('rewards')->__('Font Path (relative to store skin)'),
            'title' => Mage::helper('rewards')->__('Font Path (relative to store skin)'),
        ));
        
        $fieldset->addField('font_size', 'text', array(
            'name' => 'font_size',
            'class'     => 'validate-number',
            'label' => Mage::helper('rewards')->__('Font Size (pt)'),
            'title' => Mage::helper('rewards')->__('Font Size (pt)'),
        ));
        
        $fieldset->addField('font_color', 'text', array(
            'name' => 'font_color',
            'class'     => 'validate-number',
            'label' => Mage::helper('rewards')->__('Font Color (numeric)'),
            'title' => Mage::helper('rewards')->__('Font Color (numeric)'),
        ));
        $fieldset->addField('text_offset_x', 'text', array(
            'name' => 'text_offset_x',
            'class'     => 'validate-number',
            'label' => Mage::helper('rewards')->__('Font Left Offset'),
            'title' => Mage::helper('rewards')->__('Font Left Offset'),
        ));
        $fieldset->addField('text_offset_y', 'text', array(
            'name' => 'text_offset_y',
            'class'     => 'validate-number',
            'label' => Mage::helper('rewards')->__('Font Top Offset'),
            'title' => Mage::helper('rewards')->__('Font Top Offset'),
        ));
