 ...
 */
class TBT_Rewards_Block_Manage_Catalog_Product_Edit_Tab_Distribution
...
//        $this->collection->selectPointsCaption('points');
		...
		$this->collection->getDistriRules();
...
//        $this->addExportType('*/*/exportXml', Mage::helper('customer')->__('XML'));
    ...
    protected function _prepareLayout() {
    	$url = $this->getUrl('rewards/manage_promo_catalog/new', array( 
    					'type'=>1
    	));
    	
        $this->setChild('new_dRule',
            $this->getLayout()->createBlock('adminhtml/widget_button')
                ->setData(array(
                    'label'     => Mage::helper('adminhtml')->__('New Distribution Rule'),
                    'onclick'   => "setLocation('{$url}')",
                    'class'   => 'add'
                ))
        );
        
    	//echo "LLLLLLLLLLLLLYYYYYYYYYYYYYY";
	...
	protected function _prepareColumns()
	{
		...
		$this->addColumn('rule_id', array(
			...
			'width'     => '36px',
			'index'     => 'rule_id'
			//'filter_index'     => 'reference_table.rewards_transfer_id'
		));
		...
		$this->addColumn('points', array(
			'header'    => Mage::helper('rewards')->__('Points Amount'),
			'align'     =>'left',
			'width'     => '70px',
			'index'     => 'points_amount'
...
//						'url'       => array('base'=> 'rewards/manage_transfer/edit'),
						'url' => array('base'=> 'rewards/manage_promo_catalog/'.
									'edit/type/1'),
						'field'     => 'id'
					)
				),
    ...
    {
        return $this->getUrl(
        	'rewards/manage_catalog_product_edit/dRulesGrid', 
        	array(
        		'product_id' => $this->getProductId(),
        		'_current' => true
        	)
        ); //array('id' => $this->_getTransfer()->getId()
    }
