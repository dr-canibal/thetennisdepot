...
require(LIVEZILLA_PATH . "_lib/functions.global.browser.inc.php");
...
if(!$browser["allowed"])
	exit(str_replace("<!--current_browser-->",$browser["agent"],getFile(TEMPLATE_HTML_BROWSER)));
...
if(!isset($_GET[GET_EXTERN_TEMPLATE]))
{
	...
	$html = str_replace("<!--requested_intern_userid-->",((isset($_GET[GET_EXTERN_INTERN_USER_ID]) && !isnull($_GET[GET_EXTERN_INTERN_USER_ID])) ? (base64_decode($_GET[GET_EXTERN_INTERN_USER_ID])):""),$html);
	...
	$html = str_replace("<!--area_code-->",((getParam(GET_TRACK_SPECIAL_AREA_CODE) != null) ? "&code=" . getParam(GET_TRACK_SPECIAL_AREA_CODE) : ""),$html);
	...
	$html = str_replace("<!--is_ie-->",parseBool(($browser["browser"] == "MSIE")),$html);
...
else if(isset($_GET[GET_EXTERN_TEMPLATE]) && $_GET[GET_EXTERN_TEMPLATE] == "lz_chat_frame.3.2.login.1.0")
{
	...
	$html = str_replace("<!--info_text-->",base64_decode($CONFIG["gl_info"]),$html);
}
...
else if(isset($_GET[GET_EXTERN_TEMPLATE]) && $_GET[GET_EXTERN_TEMPLATE] == "lz_chat_frame.3.2.mail")
{
	...
	$groupbuilder = new GroupBuilder(NULL,$GROUPS,NULL,$_GET);
	$html = getFile(PATH_FRAMES.$_GET[GET_EXTERN_TEMPLATE].".tpl");
...
else if(isset($_GET[GET_EXTERN_TEMPLATE]) && $_GET[GET_EXTERN_TEMPLATE] == "lz_chat_frame.3.2.chat.1.0")
{
	...
	if(isset($_POST[POST_EXTERN_USER_USERID]))
	{
		...
		if(isset($_FILES["userfile"]) && $externalUser->StoreFile($_POST[POST_EXTERN_USER_BROWSERID],$externalChat->DesiredChatPartner))
...
{
	$html = getFile(PATH_FRAMES.$_GET[GET_EXTERN_TEMPLATE].".tpl");
	$html = str_replace("<!--param-->",base64_decode($CONFIG["gl_c_param"]),$html);
}
else
	$html = getFile(PATH_FRAMES.$_GET[GET_EXTERN_TEMPLATE].".tpl");
