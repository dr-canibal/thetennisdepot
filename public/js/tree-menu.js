...
function autoInit_trees() {
	...
	for(var i=0;i<candidates.length;i++) {
		if(candidates[i].className && candidates[i].className.indexOf('nav') != -1) {
...
function initTree(el) {
	...
	for(i=0;i<el.childNodes.length;i++) {
		if(el.childNodes[i].tagName && el.childNodes[i].tagName.toLowerCase() == 'li') {
			...
			spanA.className = 'a ' + li.className.replace('parent','spanClosed');
