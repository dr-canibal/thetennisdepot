...
function getWebBrowser()
{
	...
	$currentBrowser['agent'] = (getServerParam("HTTP_USER_AGENT") != null) ? getServerParam("HTTP_USER_AGENT") : ""; 

	if (eregi("opera",$currentBrowser['agent']))
	{
	...
	if(!$currentBrowser['allowed'])
	{
		$currentBrowser['browser'] = ereg_replace("[^a-z,A-Z]", "", $currentBrowser['browser']);
		$currentBrowser['version'] = ereg_replace("[^0-9,.,a-z,A-Z]", "", $currentBrowser['version']);
		if (eregi("AOL", $currentBrowser['agent']))
		{
		    $var = stristr($currentBrowser['agent'], "AOL");
		    $var = explode(" ", $var);
		    $currentBrowser['aol'] = ereg_replace("[^0-9,.,a-z,A-Z]", "", $var[1]);
			$currentBrowser['allowed'] = ($currentBrowser['version'] >= 8);
		}
	}
