var ExpressCheckout = Class.create();
ExpressCheckout.prototype = {
    ...
    _addEventListeners: function() {
        $('login-form') && $('login-form').observe('submit', function(e) {
            ...
            new Ajax.Request($('login-form').action, {
                ...
                onSuccess: function(transport) {
                    ExpressCheckout.Messenger.clear('login-form');

                    var response = transport.responseText.evalJSON();
                    if (response.error) {
                        ExpressCheckout.Messenger.add(response.error, 'login-form', 'error');
                    }
        ...
        $('forgot-password-form') && $('forgot-password-form').observe('submit', function(e) {
            ...
            new Ajax.Request($('forgot-password-form').action, {
                ...
                onSuccess: function(transport) {
                    ExpressCheckout.Messenger.clear('forgot-password-form');
                    ...
                    if (response.error) {
                        ExpressCheckout.Messenger.add(response.error, 'forgot-password-form', 'error');
                    } else if (response.message) {
                        expresscheckoutWindow.activate('login');
                        ExpressCheckout.Messenger.clear('login-form');
                        ExpressCheckout.Messenger.add(response.message, 'login-form', 'success');
                    }
                }
            });
        });
    ...
    save: function() {
        ...
        ExpressCheckout.Messenger.clear('checkout-review-submit');
        $$('#checkout-review-submit .checkout-agreements input[type="checkbox"]').each(function(el) {
            if (!el.checked) {
                ExpressCheckout.Messenger.add(this.acceptAgreementText, 'checkout-review-submit', 'error');
                isValid = false;
                throw $break;
            }
        }.bind(this));
...
ShippingMethod.prototype = {
    ...
    validate: function() {
        ExpressCheckout.Messenger.clear('checkout-shipping-method-load');
        var methods = document.getElementsByName('shipping_method');
        if (methods.length==0) {
            ExpressCheckout.Messenger.add(
...
Payment.prototype = {
    ...
    validate: function() {
        ExpressCheckout.Messenger.clear('checkout-payment-method-load');
...
ExpressCheckout.Window = Class.create();
ExpressCheckout.Window.prototype = {
    ...
    show: function() {
        ...
        $$('select').invoke('addClassName', 'expresscheckout-hidden');
        this.window.show();
    },

    hide: function() {
        this.window.hide();
        $$('select').invoke('removeClassName', 'expresscheckout-hidden');
    },
    ...
    _prepareMarkup: function() {
        ...
        this.window.addClassName('expresscheckout-window');
